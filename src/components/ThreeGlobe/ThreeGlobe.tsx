...
function makeArc(from: LatLng, to: LatLng, altitude = 0.2) {
  ...
  const segments = 64; // Number of points along the arc (increased for 3/4 globe arcs)
...
export default function ThreeGlobe({ 
  ...
  useEffect(() => {
    ...
    // Use square aspect ratio for consistent sizing
    ...
    // Generate arcs distributed evenly across 360 degrees
    const generateEvenlyDistributedArcs = () => {
      const arcs = [];
      const numArcs = 20;
      const longitudeStep = 360 / numArcs; // 18 degrees between each arc
      
      // Create arcs that span different longitude ranges
      for (let i = 0; i < numArcs; i++) {
        const startLng = i * longitudeStep - 180; // Start from -180 to +180
        const endLng = (startLng + 270) % 360 - 180; // End 270 degrees later (3/4 around globe)
        
        // Vary latitude for visual interest
        const startLat = (Math.sin(i * 0.5) * 60) + (Math.random() - 0.5) * 20; // -80 to +80 with variation
        const endLat = (Math.sin((i + 0.5) * 0.5) * 60) + (Math.random() - 0.5) * 20;
    ...
    const sampleLinks = generateEvenlyDistributedArcs();
    ...
    sampleLinks.forEach((link, index) => {
      ...
      const curve = makeArc(link.from, link.to, 0.1 + Math.random() * 0.08);

      // Tube with segments matching our curve points
      const tube = new THREE.TubeGeometry(curve, 64, 0.007, 16, false);
      ...
      // Larger, glowing packet spheres
      const packetGeo = new THREE.SphereGeometry(0.03, 16, 16);
      ...
      packets.push({
        ...
        curve,
        t: Math.random(),
        dt: PACKET_SPEED * (1.0 + Math.random() * 1.4),
      });
    });
  ...
  return (
    <div
      ref={wrapRef}
      className={`three-globe-wrapper three-globe ${className || ""}`}
    />
  );
}
